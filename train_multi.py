...
from models_multi import MV-DEFEAT
...
def main(cfg):
    ...
    if cfg.training.fusion_type == 'FL':
        model = FL_multi_EDL(cfg).to(device)
    else:
        model = DL_multi_EDL(cfg).to(device)
    ...
    if cfg.data.dataset_name == 'DDSM':
        train_dataset = dataset_multi.DDSM(cfg.data.root, view_laterality=cfg.data.laterality, task=cfg.data.task,
                                    split='train', transform=train__valid_transform)
        val_dataset = dataset_multi.DDSM(cfg.data.root, view_laterality=cfg.data.laterality, task=cfg.data.task, split='valid',
                                  transform=train__valid_transform)
    elif cfg.data.dataset_name == 'VindrMG':
        train_dataset = dataset_multi.VindrMG(cfg.data.root, view_laterality=cfg.data.laterality, task=cfg.data.task,
                                    split='train', transform=train__valid_transform)
        val_dataset = dataset_multi.VindrMG(cfg.data.root, view_laterality=cfg.data.laterality, task=cfg.data.task, split='valid',
                                  transform=train__valid_transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='configs_multi/vindr_density_mv_defeat.yml')
